...
import {action as unsubscribeAction} from '../routes/mailing-list.unsubscribe';
...
describe('mailing list consent and discounts',()=>{
 ...
 it('returns a retryable one-click failure while retaining local suppression',async()=>{const token=await tokenFor(env,row(),'unsubscribe');vi.stubGlobal('fetch',vi.fn(async()=>Response.json({},{status:401})));const result=await unsubscribeAction({request:new Request(`https://preview.example.com/mailing-list/unsubscribe?token=${token}`,{method:'POST',body:'List-Unsubscribe=One-Click'}),context:{cloudflare:{env}},params:{}} as any);expect((result as any).init.status).toBe(503);expect(row().status).toBe('unsubscribed');});
 ...
 it('keeps quantity and shipping separate from the product discount',async()=>{await createStripeCheckoutSession({secretKey:'test',orderId:'order',customerEmail:'test@example.com',customerId:'cus_test',promotionId:'promo_test',lines:[{kind:'custom_shirt',name:'T-shirt',unitPriceCents:2000,quantity:2},{kind:'custom_tumbler',name:'Tumbler',unitPriceCents:3500,quantity:1}],shippingCents:1000,fulfillmentMethod:'shipping',origin:'https://preview.example.com'});const c=calls[0];expect(c.body['line_items[0][quantity]']).toBe('2');expect(c.body['line_items[1][price_data][unit_amount]']).toBe('3500');expect(c.body['line_items[2][price_data][unit_amount]']).toBeUndefined();expect(c.body['shipping_options[0][shipping_rate_data][fixed_amount][amount]']).toBe('1000');expect(c.body['discounts[0][promotion_code]']).toBe('promo_test');expect(c.body.customer).toBe('cus_test');expect(c.body['line_items[0][price_data][product_data][tax_code]']).toBe('txcd_30011000');expect(c.body['line_items[1][price_data][product_data][tax_code]']).toBe('txcd_99999999');expect(c.body['line_items[0][price_data][tax_behavior]']).toBe('exclusive');expect(c.headers.get('Idempotency-Key')).toBe('checkout/order');});
});
